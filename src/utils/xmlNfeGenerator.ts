...
function base64Encode(str: string): string {
  ...
  // Fallback manual (apenas para caracteres ASCII)
  return str;
}
...
export function gerarXmlCartaCorrecao(params: {
  ...
  if (params.textoCorrecao.length < 15) {
    throw new Error('Texto de correção deve ter no mínimo 15 caracteres (TJust)');
  }
  if (params.textoCorrecao.length > 255) {
    throw new Error('Texto de correção deve ter no máximo 255 caracteres (TJust)');
  }
...
export function gerarXmlCancelamentoNFe(params: {
  ...
  if (params.justificativa.length < 15) {
    throw new Error('Justificativa deve ter no mínimo 15 caracteres (TJust)');
  }
  if (params.justificativa.length > 255) {
    throw new Error('Justificativa deve ter no máximo 255 caracteres (TJust)');
  }
