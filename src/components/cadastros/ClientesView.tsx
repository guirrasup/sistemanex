...
import { Users, Plus, Search, Edit2, Trash2, AlertCircle, Loader2, ArrowUpDown, ArrowUp, ArrowDown } from 'lucide-react';
...
export const ClientesView: React.FC<ClientesViewProps> = ({ clientes, onClientesChange }) => {
  ...
  // 🔥 COR DO MÓDULO (SKY) - MESMA DO HEADER E SIDEBAR
  const cor = 'sky';
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editandoCliente) {
        ...
        StorageService.updateCliente(editandoCliente.id, dadosCliente);
      } else {
        const novo = await clientesService.criar(dadosCliente);
      ...
      setErro(error.response?.data?.erro || error.message || 'Erro ao salvar cliente. Tente novamente.');
    } finally {
      setCarregando(false);
    }
  };
  ...
  const handleExcluir = async (id: string, razaoSocial: string) => {
    ...
    try {
      ...
      setErro(error.response?.data?.erro || error.message || 'Erro ao excluir cliente');
    } finally {
      setCarregando(false);
    }
  };
...
<td className="py-3 px-4 font-semibold">
  <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-medium ${
    ...
    c.tipo === 'AMBOS' ? 'bg-violet-100 text-violet-800' : // ✅ AMBOS = VIOLETA (cor de fornecedor)
    'bg-violet-100 text-violet-800' // FORNECEDOR = VIOLETA
  }`}>
    {c.tipo}
  </span>
</td>
