...
export class CteRepository {
  ...
  async create(data: Record<string, unknown>) {
    ...
    const payload = {
      ...
      tpImp: data.tpImp || '1',
      tpEmis: String(data.tpEmis ?? '1'),
      cDV: String(data.cDV),
      tpAmb: String(data.tpAmb ?? 1),
      ...
      indIEToma: data.indIEToma || 'NAO_CONTRIBUINTE',

      toma: data.toma || 'REMETENTE',
